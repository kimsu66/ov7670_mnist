...
module mnist_img_tb;
    ...
    // ── 픽셀 배열 ──────────────────────────────
    // Python 출력 여기에 복붙
    localparam integer result = 1; // 꼭 고쳐라..
    ...
    initial begin
        ...
        $display("    label (정답): %0d", result);  // Python label 값으로 교체
...
//                     uart_send_byte(8'h08);  // ← pixels[k] 대신 고정값
        ...
        if (result_byte == result)  // Python label 값으로 교체
            $display("    [PASS]");
        else
            $display("    [FAIL] 정답=%0d, 결과=%0d", result, result_byte);
